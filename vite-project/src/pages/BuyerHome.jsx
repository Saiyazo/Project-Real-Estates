...
const parsePriceRange = (rangeStr) => {
    ...
    if (cleanStr.includes('มากกว่า')) {
        ...
        if (cleanStr.includes('ล้าน')) minVal *= 1000000;
...
const PropertyCard = ({ property }) => {
    ...
    if (isDualPrice) {
        ...
        if (priceSection && typeof priceSection.body === 'string') {
            ...
            rentPrice = rentMatch ? Number(rentMatch[2].replace(/,/g, '')) : null;
        }
    }
        ...
        >
            ...
            <div className="card-info">
                ...
                <div className="main-tags-row tags-row-3"> 
                    {property.unitSizeSqm != null && (
                        <span className="tag size-tag">{property.unitSizeSqm} ตร.ม.</span>
                    )}
                    {formattedPricePerSqm !== 'N/A' && (
                        <span className="tag price-sqm-tag">฿{formattedPricePerSqm} /ตร.ม.</span>
                    )}
                </div>
...
const HomeListing = () => {
    ...
    const handleSearch = () => setCurrentKeyword(searchText);
    ...
    const filteredProperties = useMemo(() => {
        let listings = allListings.filter(item => {
            ...
            if (filterCriteria.dealTypes.length > 0) {
                const match = filterCriteria.dealTypes.some(type => {
                    if (type === 'sell') return item.dealType?.includes('ขาย');
                    if (type === 'rent') return item.dealType?.includes('เช่า');
                    if (type === 'lease') return item.dealType?.includes('เซ้ง');
                    return false;
                });
            ...
            if (filterCriteria.priceRange) {
                ...
                const isDual = item.dealType === 'ขายและเช่า';

                if (isDual) {
                    const priceSection = item.descriptionSections?.find(s => s.sectionId === 'price_options');
                    let rentPriceValue = null;
                    if (priceSection?.body) {
                        const rentMatch = priceSection.body.match(/(ราคาเช่า|ค่าเช่า)[\s\S]*?([\d,]+)/i);
                        rentPriceValue = rentMatch ? Number(rentMatch[2].replace(/,/g, '')) : null;
                    }
                    priceToCheck = rentPriceValue ?? item.price;
                }
    ...
    return (
        <div className="listing-page-container">
            <div className="mb-3 d-flex">
                ...
                <button className="btn btn-outline-primary ms-2" onClick={handleSearch}>Search</button>
            </div>
            ...
            <div className="filter-bar-container">
                <FilterBar onFilterChange={handleFilterChange} onClear={handleClearFilters} />
            </div>
